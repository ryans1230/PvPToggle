...
public class Toggle extends PvPCommand {
    ...
    Toggle(PvPToggle plugin, CommandSender sender) {
        super(plugin, sender);
    }
    ...
    private boolean WorldGuardRegionCheck(Player player, String target) {
        return !((Boolean) plugin.getGlobalSetting("worldguard")) || plugin.regionListener.WorldGuardRegionCheck(player, target);
    }
}