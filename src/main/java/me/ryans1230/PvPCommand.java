...
public abstract class PvPCommand {
    ...
    protected Player getPlayer(CommandSender sender, String player, boolean notify) {
        ...
        Collection<? extends Player> players = plugin.getServer().getOnlinePlayers();    // list of online players
    ...
    private String isWorld(CommandSender sender, String worldname, boolean notify) {
        ...
        if (output == null)
            if (notify) PvPLocalisation.display(sender, null, null, null, PvPLocalisation.Strings.WORLD_NOT_FOUND);
        return output;
    }
